<template>
    <div class="p-4 bg-gray-900 rounded-md shadow-lg mx-3 my-3">
        <input v-if="isEditingTitle && boardRole === 'owner' || boardRole === 'admin'" v-model="editableTitle" @blur="saveTitle" @keyup.enter="saveTitle"
          class="text-xl font-bold bg-transparent border-b border-gray-300 focus:outline-none focus:border-purple-600"
          ref="titleInput"
        />

        <h1 v-else class="flex justify-between items-center font-bold mb-2 cursor-pointer" @click="enableTitleEditing">
    ...
    <ConfirmationDialog :isVisible="isConfirmationDialogVisible" :title="$t('global.confirmDeletionTitle')" :message="$t('board.confirmMessageRemoveList')" @confirm="confirmDeletion" @cancel="cancelDeletion"/>
